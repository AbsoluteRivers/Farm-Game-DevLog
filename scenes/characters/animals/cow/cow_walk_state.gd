...
@export var cow_npc: NonPlayableCharacter
...
func _on_physics_process(_delta : float) -> void:
	...
	#
	var target_direction: Vector2 = cow_npc.global_position.direction_to(target_position)
	...
	#used velocity so the sprite doesnt flip mid avoidance
		...
		cow_npc.velocity = velocity
		cow_npc.move_and_slide()
	
	

func on_safe_velocity_computed(safe_velocity: Vector2) -> void:
	animated_sprite_2d.flip_h = safe_velocity.x < 0
	cow_npc.velocity = safe_velocity
	cow_npc.move_and_slide()



func _on_next_transitions() -> void:
	if cow_npc.current_walk_cycle == cow_npc.walk_cycle:
		cow_npc.velocity = Vector2.ZERO
		transition.emit("idle")

func _on_enter() -> void:
	animated_sprite_2d.play("walk_a")
	cow_npc.current_walk_cycle = 0
